...
const visitCreateSchema = z.object({
  ...
  listingId: z.string(),
  // if your zod < 3.22, replace .datetime() with .min(1)
...
const listingCreateSchema = z.object({
  ...
  propertyType: z.enum(["APARTMENT", "HOUSE", "VILLA", "CONDO", "TOWNHOUSE", "LAND", "OTHER"]),
  ...
  try {
    ...
    const listing = await prisma.listing.create({
      data: {
        ...
        // adjust relation name if needed (createdBy vs createdById)
        createdBy: {
          connectOrCreate: {
...
app.get("/listings", async (req, res, next) => {
  try {
    const { city, category, minPrice, maxPrice, bedrooms, propertyType, take, skip } = req.query;
...
app.get("/visits", async (req, res, next) => {
  try {
    ...
    const where = {
      ...
      ...(from || to
        ? { scheduledAt: { ...(from ? { gte: new Date(String(from)) } : {}), ...(to ? { lte: new Date(String(to)) } : {}) } }
        : {}),
    };
...
app.patch("/visits/:id", async (req, res, next) => {
  try {
    const { id } = req.params;
